...
class FirebaseAuthManager extends AuthManager
  ...
  Future resetPassword({
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text('خطأ:[error]'.replaceAll('[error]', e.message!))),
      );
      return null;
    }
    ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : MuhtarFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      final errorMsg = switch (e.code) {
        'email-already-in-use' => '! الايميل مستخدم بالفعل',
        'INVALID_LOGIN_CREDENTIALS' => 'البيانات المدخلة غير صحيحة',
        _ => 'خطأ:[error]'.replaceAll('[error]', e.message!),
      };
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
